...
import { User, Mail, ImageIcon, Info, Pencil } from "lucide-react";
        ...
        {user && (
          <div className="flex flex-col items-center gap-6 md:flex-row">
            <div className="group relative flex h-48 w-48 items-center justify-center overflow-hidden rounded-full border p-2">
              {user.avatar ? <Image src={user.avatar.url || ""} alt="avatar" fill priority className="object-cover object-center" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" /> : <ImageIcon className="absolute left-1/2 top-1/2 h-10 w-10 -translate-x-1/2 -translate-y-1/2 text-primary" />}
              <div className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity group-hover:opacity-100">
                <Pencil className="h-8 w-8 text-white" />
              </div>
              <label htmlFor="avatar" className="absolute inset-0 cursor-pointer" onClick={() => document.getElementById("avatar")?.click()} />
